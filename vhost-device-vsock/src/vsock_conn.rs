            ...
            Some(RxOps::Rw) => {
                ...
                match self.stream.read_volatile(&mut buf) {
                        ...
                        if e.kind() == ErrorKind::WouldBlock =>
                    {
                        ...
                        // available in the socket right now. We should stop trying to read
                        // and re-enable EPOLLIN to wait for more data.
                        // We also need to tell the caller that this virtio descriptor
                        // should not be returned to the guest yet, because we haven't
                        // put any data in it. The easiest way is to return an error.
                        // The caller (`process_rx_queue`) will see the error and stop
                        // processing descriptors for this queue for now.
                        if VhostUserVsockThread::epoll_modify(
                            self.epoll_fd,
                            self.stream.as_raw_fd(),
                            epoll::Events::EPOLLIN,
                        ...
                        return Err(Error::NoRequestRx); // Signal to stop processing
                    }
                    Err(e) => {
